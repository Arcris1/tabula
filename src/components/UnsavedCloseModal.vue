...
<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60" @click.self="emit('cancel')">
    <div class="w-96 rounded border border-zinc-700 bg-zinc-900 p-4">
      <div class="text-sm font-medium mb-1">Unsaved query</div>
      ...
      <input ref="input" v-model="name" placeholder="e.g. stuck leads last 24h"
        @keydown.enter.prevent="save" @keydown.esc.prevent="emit('cancel')"
      ...
      <div class="flex justify-end gap-2 mt-4">
        ...
        <button @click="save" :disabled="!name.trim()"
          class="px-3 py-1.5 rounded text-xs text-white bg-blue-600 hover:bg-blue-500 disabled:opacity-40">Save &amp; close</button>
      </div>
    </div>
  </div>
</template>
